...
describe('AuthService', () => {
  ...
  describe('Signin', () => {
    ...
    it('should create a new user and return user data with access_token', async () => {
      ...
      expect(findUniqueSpy).toHaveBeenCalledWith({ where: { email: authDto.email } });
      expect(createSpy).toHaveBeenCalledWith({ data: { ...authDto } });
      expect(signAsyncSpy).toHaveBeenCalledWith({ uid: createdUser.uid, name: createdUser.name, email: createdUser.email, idUserAd: createdUser.idUserAd });
      expect(result).toEqual({ user: createdUser, access_token: 'mockAccessToken' });
    });
    ...
    it('should return user data with access_token for an existing user', async () => {
      ...
      expect(findUniqueSpy).toHaveBeenCalledWith({ where: { email: authDto.email } });
      expect(signAsyncSpy).toHaveBeenCalledWith({ uid: existingUser.uid, name: existingUser.name, email: existingUser.email, idUserAd: existingUser.idUserAd });
      expect(result).toEqual({ user: existingUser, access_token: 'mockAccessToken' });
    });

    it('should handle errors when creating a user', async () => {
      findUniqueSpy.mockResolvedValue(null);
      createSpy.mockImplementation(() => { throw new Error('Some error'); });

      await expect(service.signin(authDto)).rejects.toThrowError(new InternalServerErrorException('Some error'));
    });

    it('should handle errors when signing the token', async () => {
      findUniqueSpy.mockResolvedValue(null);
      createSpy.mockResolvedValue(createdUser);
      signAsyncSpy.mockImplementation(() => { throw new Error('Token error'); });

      await expect(service.signin(authDto)).rejects.toThrowError(new InternalServerErrorException('Token error'));
    });
  });
});
